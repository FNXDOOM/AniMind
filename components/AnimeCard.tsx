...
const AnimeCard: React.FC<AnimeCardProps> = ({ 
  ...
  const getStatusColor = (status?: WatchStatus) => {
    switch (status) {
      case 'Watching': return 'bg-primary text-black font-bold';
      case 'Completed': return 'bg-secondary text-white font-bold';
      case 'Dropped': return 'bg-red-500 text-white font-bold';
      ...
      <div className="relative aspect-[2/3] overflow-hidden bg-black">
        ...
        {onRemove ? (
            ...
            <button 
                ...
                className={`absolute top-2 left-2 p-2 rounded-full backdrop-blur-md border transition-colors ${isInWatchlist ? 'bg-primary text-black border-primary' : 'bg-black/40 border-white/10 text-white hover:bg-white/20 hover:text-primary'}`}
            >
