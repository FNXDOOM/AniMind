...
const Layout: React.FC<LayoutProps> = ({ children, currentView, onChangeView, user, onLogout, onDeleteAccount }) => {
  ...
  return (
    <div className="min-h-screen bg-background text-white flex flex-col md:flex-row font-sans">
      ...
      {/* Sidebar Navigation */}
      <aside className={`
        fixed inset-y-0 left-0 z-40 w-64 bg-surface border-r border-white/5 transform transition-transform duration-300 ease-in-out md:translate-x-0 md:relative md:block flex flex-col
      ...
      {/* Main Content */}
      <main className="flex-1 overflow-y-auto h-[calc(100vh-60px)] md:h-screen scroll-smooth bg-background">
